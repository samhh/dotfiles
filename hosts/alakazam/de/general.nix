...
let
  ...
  locker = import ./pkgs/locker.nix { inherit pkgs; };
  wallpaper = import ./pkgs/wallpaper.nix { inherit config lib pkgs; };
in
{
  ...
  home-manager.users.${config.username} = {
    wayland.windowManager.sway =
      let
        mod = "Mod4";
        mprisPlayers = lib.concatStringsSep "," [ "mpv" "firefox" "mpd" ];
      in
      with lib; {
        enable = true;
